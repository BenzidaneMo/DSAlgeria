...
export default function MainWorkspace({ selectedAlgorithm, onSelectAlgorithm, currentStep, array, onApplyArray }) {
  ...
  return (
    <main className="relative grid min-h-0 flex-1 grid-cols-[minmax(190px,0.8fr)_minmax(0,2.2fr)_minmax(210px,0.9fr)] overflow-hidden border-b border-border" aria-label="مساحة العمل الرئيسية">
        ...
        <div className="absolute bottom-4 left-4 flex items-center justify-center w-xs gap-2 border border-border-subtle p-1.5">
            ...
            <button type="button" onClick={() => setIsArrayEditorOpen(true)} className="w-34 h-fit flex items-center justify-center gap-1 rounded-md border border-border-subtle py-4 px-2.5  hover:bg-bg-hover">
                <Pencil className="h-3 w-3" />
                <span className="w-fit text-[11px] text-text-secondary hover:text-text-primary">تعديل المصفوفة</span>
            </button>
        </div>
